   ...
   if(! isSet){
        ...
        .then( flight => {
          ...
          setFlight({...flight.data[0],
                      departure_time:format(Date.parse(flight.data[0].departure_time),"PPPPp"),
                      arrival_time:format(Date.parse(flight.data[0].arrival_time),"PPPPp")
                    });
    ...
    return (
      ...
      <>
      <hr />
      <h3 className="text-center">{props.header} | From:  {flight.from}  To:  {flight.to}</h3>
